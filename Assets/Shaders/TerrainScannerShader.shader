Shader "TAP/TerrainScannerShader"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // ── Global uniforms (set by TerrainScannerController.cs) ──
            float4 _ScannerOrigin;      // world-space XYZ origin da onda
            float  _ScannerRadius;      // raio atual da onda
            float  _ScannerMaxRadius;   // raio máximo (para fade)
            float  _ScannerActive;      // 0 = inativo, 1 = ativo
            float  _ScannerRevealItems; // 0 = não revela, 1 = revela
            ...
            {
                float f = frac(v - 0.5) - 0.5;   // -0.5 .. 0.5
                return 1.0 - smoothstep(0.0, lw, abs(f));
            }

            // Grid triplanar: cruza linhas paralelas nos 3 eixos
            float TriplanarGrid(float3 wpos, float3 wnorm, float scale, float lw)
            {
                ...
                float gx = max(GridLine(wpos.y * scale, lw),
                               GridLine(wpos.z * scale, lw));
                float gy = max(GridLine(wpos.x * scale, lw),
                               GridLine(wpos.z * scale, lw));
                float gz = max(GridLine(wpos.x * scale, lw),
                               GridLine(wpos.y * scale, lw));

                return saturate(gx * blend.x + gy * blend.y + gz * blend.z);
            }

            // Linhas de contorno horizontais (altitude)
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                // ── 1. Base lighting (Lambert simples) ────────────────
                ...
                // ── 4. Trail que fica atrás da onda ───────────────────
                float  trailDist  = _ScannerRadius - dist;         // positivo atrás da onda
                float  trailFade  = saturate(trailDist / _WaveGlowRange);
                trailFade = pow(trailFade, 1.8) * inside * radiusFade;

                // ── 5. Grid triplanar (dentro da área varrida) ────────
                float  grid    = TriplanarGrid(i.worldPos, N,
                                               _GridScale, _GridLineWidth);
                float3 gridCol = _GridColor.rgb * grid * _GridIntensity * trailFade;

                // ── 6. Linhas de contorno (altitude) ──────────────────
                float  contour    = ContourLine(i.worldPos.y,
                                                _ContourSpacing, _ContourWidth);
                float3 contourCol = _ContourColor.rgb * contour
                                  * _ContourIntensity * trailFade;

                // ── 7. Reveal de itens escondidos ─────────────────────
                ...
                // ── 8. Composição final ───────────────────────────────
                float  scanActive = _ScannerActive;
                float3 finalCol   = lit
        ...
        // Shadow caster pass (necessário para receber sombras corretamente)
        Pass
        {
            ...
            // ATENÇÃO: o parâmetro DEVE chamar-se "v" e os campos "vertex"/"normal"
